...
class BQW_Grid_Accordion_Lite {
	 ...
	 * @var string
	 */
	protected $plugin_slug = 'grid-accordion-lite';
	...
	public function load_plugin_textdomain() {
		...
		load_plugin_textdomain( $domain, false, 'grid-accordion-lite/languages/' );
	}
	...
	public function register_styles() {
		wp_register_style( $this->plugin_slug . '-plugin-style', plugins_url( 'grid-accordion-lite/public/assets/css/grid-accordion.min.css' ), array(), self::VERSION );
	}
	...
	public function register_scripts() {
		wp_register_script( $this->plugin_slug . '-plugin-script', plugins_url( 'grid-accordion-lite/public/assets/js/jquery.gridAccordion.min.js' ), array( 'jquery' ), self::VERSION );
	}
	...
	public function output_accordion( $accordion_data, $allow_cache = true ) {
		$accordion = new BQW_GAL_Accordion_Renderer( $accordion_data );
	...
	public function load_styles() {
		...
		if ( $this->styles_loaded === false && is_active_widget( false, false, 'bqw-grid-accordion-lite-widget' ) ) {
			$this->styles_loaded = true;
		}
	...
	public function grid_accordion_shortcode( $atts, $content = null ) {
		...
		$styles_warning = '<div class="ga-styles-warning" style="width: 450px; background-color: #FFF; color: #F00; border: 1px solid #F00; padding: 10px; font-size: 14px;">
			...
			You can set that <a style="text-decoration: underline; color: #F00;" href="' . admin_url( 'admin.php?page=grid-accordion-lite-settings' ) . '">here</a>.
			</div>';
