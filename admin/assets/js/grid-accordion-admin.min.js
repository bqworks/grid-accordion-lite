...
".preview-accordion",function(c){c.preventDefault();b.previewAccordionAll(a(this))});a(".accordions-list").on("click",".delete-accordion",function(c){c.preventDefault();b.deleteAccordion(a(this))});a(".accordions-list").on("click",".duplicate-accordion",function(c){c.preventDefault();b.duplicateAccordion(a(this))});a(".clear-all-cache").on("click",function(c){c.preventDefault();a(".clear-cache-spinner").css({display:"inline-block",visibility:"visible"});c=a(this).attr("data-nonce");a.ajax({url:ga_js_vars.ajaxurl,
type:"post",data:{action:"grid_accordion_lite_clear_all_cache",nonce:c},complete:function(c){a(".clear-cache-spinner").css({display:"",visibility:""})}})})},loadAccordionData:function(){var b=this;a(".panel-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,type:"get",data:{action:"grid_accordion_lite_get_accordion_data",id:ga_js_vars.id,nonce:ga_js_vars.lad_nonce},complete:function(c){c=a.parseJSON(c.responseText);a.each(c.panels,function(c,e){var k={background:{},
layers:e.layers,html:e.html,settings:a.isArray(e.settings)?{}:e.settings};a.each(e,function(a,c){-1!==a.indexOf("background")&&(k.background[a]=c)});b.getPanel(c).setData("all",k)});a(".panel-spinner").css({display:"",visibility:""})}})},saveAccordion:function(){var b=this.getAccordionData();b.nonce=ga_js_vars.sa_nonce;b.action="save";var b=JSON.stringify(b),c=a(".update-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_save_accordion",
data:b},complete:function(b){c.css({display:"",visibility:""});-1===parseInt(ga_js_vars.id,10)&&!1===isNaN(b.responseText)?(a("h2").after('<div class="updated"><p>'+ga_js_vars.accordion_create+"</p></div>"),window.location=ga_js_vars.admin+"?page=grid-accordion-lite&id="+b.responseText+"&action=edit"):0===a(".updated").length&&a("h2").after('<div class="updated"><p>'+ga_js_vars.accordion_update+"</p></div>")}})},getAccordionData:function(){var b=this,c={id:ga_js_vars.id,name:a("input#title").val(),
settings:{},panels:[],panels_state:{}};a(".panels-container").find(".panel").each(function(f){var e=a(this),k=b.getPanel(parseInt(e.attr("data-id"),10)).getData("all");k.position=parseInt(e.attr("data-position"),10);c.panels[f]=k});a(".sidebar-settings").find(".setting").each(function(){var b=a(this);c.settings[b.attr("name")]="checkbox"===b.attr("type")?b.is(":checked"):b.val()});a(".sidebar-settings").find(".postbox").each(function(){var b=a(this);c.panels_state[b.attr("data-name")]=b.hasClass("closed")?
"closed":""});return c},previewAccordion:function(){h.open(this.getAccordionData())},previewAccordionAll:function(b){var c=a.lightURLParse(b.attr("href"));b=c.lad_nonce;c=parseInt(c.id,10);a.ajax({url:ga_js_vars.ajaxurl,type:"get",data:{action:"grid_accordion_lite_get_accordion_data",id:c,nonce:b},complete:function(b){b=a.parseJSON(b.responseText);h.open(b)}})},deleteAccordion:function(b){var c=a.lightURLParse(b.attr("href")),f=c.da_nonce,e=parseInt(c.id,10),k=b.parents("tr"),d=a('<div class="modal-overlay"></div><div class="modal-window-container">\t<div class="modal-window delete-accordion-dialog">\t\t<p class="dialog-question">'+
...
accordion:null,accordionData:null,open:function(b){this.accordionData=b;var c=this,d=a(".preview-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_preview_accordion",data:JSON.stringify(b)},complete:function(b){a("body").append(b.responseText);c.init();d.css({display:"",visibility:""})}})},init:function(){var b=this;a(".modal-window-container").css("top",a(window).scrollTop());this.previewWindow=a(".preview-window .modal-window");
this.accordion=this.previewWindow.find(".grid-accordion");this.previewWindow.find(".close-x").on("click",function(a){b.close()});var c=this.accordionData.settings.width,d=this.accordionData.settings.height,e=-1!==c.indexOf("%"),h=-1!==d.indexOf("%");!0===e?this.accordion.gridAccordion("width","100%"):c=parseInt(c,10);!0===h&&this.accordion.gridAccordion("height","100%");a(window).on("resize.gridAccordion",function(){!0===e?b.previewWindow.css("width",a(window).width()*(parseInt(c,10)/100)-100):c>=
a(window).width()-100?b.previewWindow.css("width",a(window).width()-100):b.previewWindow.css("width",c);!0===h&&b.previewWindow.css("height",a(window).height()*(parseInt(d,10)/100)-200)});a(window).trigger("resize");a(window).trigger("resize")},close:function(){this.previewWindow.find(".close-x").off("click");a(window).off("resize.gridAccordion");this.accordion.gridAccordion("destroy");a("body").find(".modal-overlay, .modal-window-container").remove()}};a(document).ready(function(){l.init()})})(jQuery);
