(function(a){var l={panels:[],panelCounter:0,postsData:{},allowPanelImageResize:!0,init:function(){"single"===ga_js_vars.page?this.initSingleAccordionPage():"all"===ga_js_vars.page&&this.initAllAccordionsPage()},initSingleAccordionPage:function(){var c=this;this.initPanels();-1!==parseInt(ga_js_vars.id,10)&&this.loadAccordionData();a("form").on("submit",function(a){a.preventDefault();c.saveAccordion()});a(".preview-accordion").on("click",function(a){a.preventDefault();c.previewAccordion()});a(".add-panel").on("click",
...
a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_clear_all_cache",nonce:b},complete:function(b){a(".clear-cache-spinner").css({display:"",visibility:""})}})})},loadAccordionData:function(){var c=this;a(".panel-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,type:"get",data:{action:"grid_accordion_lite_get_accordion_data",id:ga_js_vars.id,nonce:ga_js_vars.lad_nonce},complete:function(b){b=a.parseJSON(b.responseText);a.each(b.panels,
function(b,e){var k={background:{},layers:e.layers,html:e.html,settings:a.isArray(e.settings)?{}:e.settings};a.each(e,function(a,b){-1!==a.indexOf("background")&&(k.background[a]=b)});c.getPanel(b).setData("all",k)});a(".panel-spinner").css({display:"",visibility:""})}})},saveAccordion:function(){var c=this.getAccordionData();c.nonce=ga_js_vars.sa_nonce;c.action="save";var c=JSON.stringify(c),b=a(".update-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,
type:"post",data:{action:"grid_accordion_lite_save_accordion",data:c},complete:function(c){b.css({display:"",visibility:""});-1===parseInt(ga_js_vars.id,10)&&!1===isNaN(c.responseText)?(a("h2").after('<div class="updated"><p>'+ga_js_vars.accordion_create+"</p></div>"),window.location=ga_js_vars.admin+"?page=grid-accordion-lite&id="+c.responseText+"&action=edit"):0===a(".updated").length&&a("h2").after('<div class="updated"><p>'+ga_js_vars.accordion_update+"</p></div>")}})},getAccordionData:function(){var c=
this,b={id:ga_js_vars.id,name:a("input#title").val(),settings:{},panels:[],panels_state:{}};a(".panels-container").find(".panel").each(function(f){var e=a(this),k=c.getPanel(parseInt(e.attr("data-id"),10)).getData("all");k.position=parseInt(e.attr("data-position"),10);b.panels[f]=k});a(".sidebar-settings").find(".setting").each(function(){var c=a(this);b.settings[c.attr("name")]="checkbox"===c.attr("type")?c.is(":checked"):c.val()});a(".sidebar-settings").find(".postbox").each(function(){var c=a(this);
b.panels_state[c.attr("data-name")]=c.hasClass("closed")?"closed":""});return b},previewAccordion:function(){h.open(this.getAccordionData())},previewAccordionAll:function(c){var b=a.lightURLParse(c.attr("href"));c=b.lad_nonce;b=parseInt(b.id,10);a.ajax({url:ga_js_vars.ajaxurl,type:"get",data:{action:"grid_accordion_lite_get_accordion_data",id:b,nonce:c},complete:function(b){b=a.parseJSON(b.responseText);h.open(b)}})},deleteAccordion:function(c){var b=a.lightURLParse(c.attr("href")),f=b.da_nonce,e=
parseInt(b.id,10),k=c.parents("tr"),d=a('<div class="modal-overlay"></div><div class="modal-window-container">\t<div class="modal-window delete-accordion-dialog">\t\t<p class="dialog-question">'+ga_js_vars.accordion_delete+'</p>\t\t<div class="dialog-buttons">\t\t\t<a class="button dialog-ok" href="#">'+ga_js_vars.yes+'</a>\t\t\t<a class="button dialog-cancel" href="#">'+ga_js_vars.cancel+"</a>\t\t</div>\t</div></div>").appendTo("body");a(".modal-window-container").css("top",a(window).scrollTop());
d.find(".dialog-ok").one("click",function(b){b.preventDefault();a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_delete_accordion",id:e,nonce:f},complete:function(a){e===parseInt(a.responseText,10)&&k.fadeOut(300,function(){k.remove()})}});d.remove()});d.find(".dialog-cancel").one("click",function(a){a.preventDefault();d.remove()});d.find(".modal-overlay").one("click",function(a){d.remove()})},duplicateAccordion:function(c){var b=a.lightURLParse(c.attr("href"));c=b.dua_nonce;
b=parseInt(b.id,10);a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_duplicate_accordion",id:b,nonce:c},complete:function(b){a(b.responseText).appendTo(a(".accordions-list tbody")).hide().fadeIn()}})},initPanels:function(){var c=this;a(".panels-container").find(".panel").each(function(b){c.initPanel(a(this))});a(".panels-container").lightSortable({children:".panel",placeholder:"panel panel-placeholder",sortEnd:function(b){a(".panel").each(function(b){a(this).attr("data-position",
b)})}})},initPanel:function(a,b){var f=this,e=new d(a,this.panelCounter,b);this.panels.push(e);e.on("duplicatePanel",function(a){f.duplicatePanel(a.panelData)});e.on("deletePanel",function(a){f.deletePanel(a.id)});a.attr("data-id",this.panelCounter);a.attr("data-position",this.panelCounter);this.panelCounter++},getPanel:function(c){var b;a.each(this.panels,function(a,e){if(e.id===c)return b=e,!1});return b},duplicatePanel:function(c){var b=this,f=a.extend(!0,{},c);a.ajax({url:ga_js_vars.ajaxurl,type:"post",
data:{action:"grid_accordion_lite_add_panels",data:JSON.stringify([{settings:{content_type:f.settings.content_type},background_source:f.background.background_source}])},complete:function(c){c=a(c.responseText).appendTo(a(".panels-container"));b.initPanel(c,f)}})},deletePanel:function(c){var b=this,f=b.getPanel(c),e=a('<div class="modal-overlay"></div><div class="modal-window-container">\t<div class="modal-window delete-panel-dialog">\t\t<p class="dialog-question">'+ga_js_vars.panel_delete+'</p>\t\t<div class="dialog-buttons">\t\t\t<a class="button dialog-ok" href="#">'+
ga_js_vars.yes+'</a>\t\t\t<a class="button dialog-cancel" href="#">'+ga_js_vars.cancel+"</a>\t\t</div>\t</div></div>").appendTo("body");a(".modal-window-container").css("top",a(window).scrollTop());e.find(".dialog-ok").one("click",function(c){c.preventDefault();f.off("duplicatePanel");f.off("deletePanel");f.remove();e.remove();b.panels.splice(a.inArray(f,b.panels),1)});e.find(".dialog-cancel").one("click",function(a){a.preventDefault();e.remove()});e.find(".modal-overlay").one("click",function(a){e.remove()})},
addImagePanels:function(){var c=this;g.open(function(b){var f=[];a.each(b,function(a,b){f.push({background_source:b.url,background_alt:b.alt,background_title:b.title,background_width:b.width,background_height:b.height,background_link:b.link})});a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_add_panels",data:JSON.stringify(f)},complete:function(b){var d=a(".panels-container").find(".panel").length-1;a(".panels-container").append(b.responseText).find(".panel"+(-1===d?"":
...
wp.media.editor.insert=function(e){a.call(this,b);wp.media.editor.insert=d};wp.media.editor.open("media-loader")}},h={previewWindow:null,accordion:null,accordionData:null,open:function(c){this.accordionData=c;var b=this,d=a(".preview-spinner").css({display:"inline-block",visibility:"visible"});a.ajax({url:ga_js_vars.ajaxurl,type:"post",data:{action:"grid_accordion_lite_preview_accordion",data:JSON.stringify(c)},complete:function(c){a("body").append(c.responseText);b.init();d.css({display:"",visibility:""})}})},
init:function(){var c=this;a(".modal-window-container").css("top",a(window).scrollTop());this.previewWindow=a(".preview-window .modal-window");this.accordion=this.previewWindow.find(".grid-accordion");this.previewWindow.find(".close-x").on("click",function(a){c.close()});var b=this.accordionData.settings.width,d=this.accordionData.settings.height,e=-1!==b.indexOf("%"),h=-1!==d.indexOf("%");!0===e?this.accordion.gridAccordion("width","100%"):b=parseInt(b,10);!0===h&&this.accordion.gridAccordion("height",
"100%");a(window).on("resize.gridAccordion",function(){!0===e?c.previewWindow.css("width",a(window).width()*(parseInt(b,10)/100)-100):b>=a(window).width()-100?c.previewWindow.css("width",a(window).width()-100):c.previewWindow.css("width",b);!0===h&&c.previewWindow.css("height",a(window).height()*(parseInt(d,10)/100)-200)});a(window).trigger("resize");a(window).trigger("resize")},close:function(){this.previewWindow.find(".close-x").off("click");a(window).off("resize.gridAccordion");this.accordion.gridAccordion("destroy");
