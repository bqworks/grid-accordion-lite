 ...
 * @since 1.0.0
 */
class BQW_Grid_Accordion_Lite_Admin {
	...
	private function __construct() {
		$this->plugin = BQW_Grid_Accordion_Lite::get_instance();
		...
		add_action( 'wp_ajax_grid_accordion_lite_get_accordion_data', array( $this, 'ajax_get_accordion_data' ) );
		add_action( 'wp_ajax_grid_accordion_lite_save_accordion', array( $this, 'ajax_save_accordion' ) );
		add_action( 'wp_ajax_grid_accordion_lite_preview_accordion', array( $this, 'ajax_preview_accordion' ) );
		add_action( 'wp_ajax_grid_accordion_lite_delete_accordion', array( $this, 'ajax_delete_accordion' ) );
		add_action( 'wp_ajax_grid_accordion_lite_duplicate_accordion', array( $this, 'ajax_duplicate_accordion' ) );
		add_action( 'wp_ajax_grid_accordion_lite_add_panels', array( $this, 'ajax_add_panels' ) );
		add_action( 'wp_ajax_grid_accordion_lite_clear_all_cache', array( $this, 'ajax_clear_all_cache' ) );
	}
	...
	public function enqueue_admin_styles() {
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffixes ) ) {
			wp_enqueue_style( $this->plugin_slug . '-admin-style', plugins_url( 'grid-accordion-lite/admin/assets/css/grid-accordion-admin.min.css' ), array(), BQW_Grid_Accordion_Lite::VERSION );
			wp_enqueue_style( $this->plugin_slug . '-plugin-style', plugins_url( 'grid-accordion-lite/public/assets/css/grid-accordion.min.css' ), array(), BQW_Grid_Accordion_Lite::VERSION );
		}
	}
	...
	public function enqueue_admin_scripts() {
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffixes ) ) {
			...
			wp_enqueue_script( $this->plugin_slug . '-admin-script', plugins_url( 'grid-accordion-lite/admin/assets/js/grid-accordion-admin.min.js' ), array( 'jquery' ), BQW_Grid_Accordion_Lite::VERSION );
			wp_enqueue_script( $this->plugin_slug . '-plugin-script', plugins_url( 'grid-accordion-lite/public/assets/js/jquery.gridAccordion.min.js' ), array( 'jquery' ), BQW_Grid_Accordion_Lite::VERSION );
	...
	public function add_admin_menu() {
		$plugin_settings = BQW_Grid_Accordion_Lite_Settings::getPluginSettings();
		...
		add_menu_page(
			...
			plugins_url( '/grid-accordion-lite/admin/assets/css/images/ga-icon.png' )
		);
	...
	public function save_accordion( $accordion_data ) {
		...
		foreach ( $panels_data as $panel_data ) {
			...
			$wpdb->insert( $wpdb->prefix . 'gridaccordion_panels', $panel, array( '%d', '%s', '%d', '%s', '%s', '%s', '%s', '%d', '%d', '%s', '%s', '%s' ) );
		}
